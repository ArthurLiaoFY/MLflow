...
def cross_entropy_loss(
        y_pred: torch.Tensor, y_true: torch.Tensor, weight: torch.Tensor | None = None, reduction: str = 'mean'
) -> torch.Tensor:
    weight = torch.ones_like(y_true) * weight / weight.sum() if weight is not None else torch.ones_like(y_true)
    if reduction == 'sum':
...
def focal_loss(y_pred: torch.Tensor, y_true: torch.Tensor, gamma: float = 1.0, reduction: str = 'mean') -> torch.Tensor:
...
def fuzzy_root_mean_square_error(y_pred: torch.Tensor, y_true: torch.Tensor, epsilon: float = 0.1) -> torch.Tensor:
    # y_pred[0] - y_true < 0 then epsilon * pow
    # y_true - y_pred[2] < 0 then epslion * pow
    return (
        # torch.sqrt(torch.pow((y_pred[:, 1] - y_true), 2).mean()) + 
        torch.sqrt((torch.where(y_true - y_pred[:, 0] > 0.0, epsilon, 1.0) * torch.pow((y_pred[:, 0] - y_true), 2)).mean()) +
        torch.sqrt((torch.where(y_pred[:, 1] - y_true > 0.0, epsilon, 1.0) * torch.pow((y_pred[:, 1] - y_true), 2)).mean())
    )
...
def huber_loss(y_pred: torch.Tensor, y_true: torch.Tensor, delta: float = 0.5, reduction: str = 'mean') -> torch.Tensor:
    dist = (y_pred - y_true).abs()
    indicator = (dist <= delta).to(torch.float)
    if reduction == 'sum':
        return (0.5 * torch.pow(dist, 2) * indicator + delta * (dist - delta * 0.5) * (1.0 - indicator)).sum()
    else:
        return (0.5 * torch.pow(dist, 2) * indicator + delta * (dist - delta * 0.5) * (1.0 - indicator)).sum()


def var_explain_rate(y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
    return torch.pow((y_true - y_pred), 2).sum() / torch.pow((y_true - y_true.mean()), 2).sum()
