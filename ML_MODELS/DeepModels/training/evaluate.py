...
class RSquare:
    def __init__(self):
        ...
        self.RSE, self.SSE = torch.tensor([0.0]).to(self.device), torch.tensor([0.0]).to(self.device)
...
class Accuracy:
    def __init__(self):
        self.device = get_device()
        self.y_pred_idx, self.y_true_idx = torch.tensor([]).to(self.device), torch.tensor([]).to(self.device)
...
class Recall:
    ...
    def finish(self) -> torch.Tensor:
        recall = (self.y_pred_idx[self.y_true_idx == 0] == self.y_true_idx[self.y_true_idx == 0]).sum() / (self.y_true_idx == 0).sum()
        self.initialize()
        return recall
...
class Precision:
    ...
    def finish(self) -> torch.Tensor:
        precision = (self.y_pred_idx[self.y_true_idx == 0] == self.y_true_idx[self.y_true_idx == 0]).sum() / (self.y_pred_idx == 0).sum()
        self.initialize()
        return precision
