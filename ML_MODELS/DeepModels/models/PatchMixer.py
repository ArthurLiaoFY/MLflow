...
class PatchMixerModel(torch.nn.Module):
    def __init__(self, train_seq_len: int, test_seq_len: int, patch_len: int = 16, stride: int = 8):
        ...
        self.patch_num = int((self.train_seq_len - self.patch_len) / self.stride + 1) + 1
        ...
        self.linear_flatten_head = torch.nn.Sequential(
            torch.nn.Linear(in_features=self.patch_num * self.d_model, out_features=self.test_seq_len),
            torch.nn.Dropout(0.3),
        )

        self.residual_conn_layer = torch.nn.Linear(self.d_model, self.patch_len)
        self.depthwise_conv_block = torch.nn.Sequential(
            torch.nn.Conv1d(self.patch_num, self.patch_num, kernel_size=self.patch_len, stride=self.patch_len,
        ...
        self.pointwise_conv_block = torch.nn.Sequential(
            torch.nn.Conv1d(self.patch_num, self.patch_num, kernel_size=1, stride=1,
                            groups=self.patch_num),
            torch.nn.GELU(),
            torch.nn.BatchNorm1d(self.patch_num),
        )
        self.mlp_flatten_head = torch.nn.Sequential(
            torch.nn.Linear(in_features=self.patch_num * self.patch_len, out_features=256),
            torch.nn.GELU(),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        z = z.reshape(batch * feature_amount, self.patch_num, self.patch_len, 1).squeeze(-1)
        # embedding
        z = self.embedding(z)
