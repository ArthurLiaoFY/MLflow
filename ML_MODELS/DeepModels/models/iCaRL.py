...
class RepresentativeSample:
    def __init__(self, max_datasize: int = 2000, seed: int | None = 1122, method: str = 'mean'):
        self.max_datasize = max_datasize
        self.seed = seed
        self.method = method

    def construct_sample_set(self,
        ...
        for col_idx in range(y.shape[1]):
            ...
            # save class mean
            result[col_idx]['class_center'] = y_pred_masked.mean(dim=0) if self.method == 'mean' else y_pred_masked.median(dim=0)

            for k in range(min(samples_per_class, y_pred_masked.shape[0])):
                if k == 0:
                    distance = get_norm(x=y_pred_masked - result[col_idx]['class_center'], norm=norm)
                    result[col_idx]['representative_sample'] = y_pred_masked[torch.argmin(distance)][None, :]
                    y_pred_masked = y_pred_masked[
                                    ~torch.all(input=y_pred_masked == y_pred_masked[torch.argmin(distance)], dim=1), :]
