...
class Convolutional1DGRUAttention(torch.nn.Module):
    def __init__(
        ...
        self.conv_1d_block = torch.nn.Sequential(
            torch.nn.Conv1d(in_channels=conv_in_channels, out_channels=conv_out_channels, kernel_size=3, padding='same'),
        ...
        self.gru_layer = torch.nn.GRU(input_size=gru_input_size, hidden_size=gru_hidden_size, num_layers=gru_layer_amount, bias=True,
                                      batch_first=True, dropout=0.4, bidirectional=False)
        ...
        self.q_conv_layer = torch.nn.Sequential(
            torch.nn.Conv1d(in_channels=conv_out_channels, out_channels=conv_out_channels, kernel_size=3, padding='same'),
        ...
        self.final_layer = torch.nn.Sequential(
            torch.nn.Linear(in_features=conv_out_channels*gru_hidden_size, out_features=256),
    ...
    def forward(self, input_tensor: torch.Tensor):
        ...
        init_hidden_state = torch.zeros(size=(self.gru_layer_amount, batch_size, self.gru_hidden_size)).to(self.device)
