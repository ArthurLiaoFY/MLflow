...
class PairwiseCorrelation(torch.nn.Module):
    ...
    def forward(self, target_x: torch.Tensor, target_y: torch.Tensor):
        ...
        return (demean_target_x * demean_target_y).sum(dim=1) / (torch.sqrt(torch.pow(input=demean_target_x, exponent=2).sum(dim=1)) * torch.sqrt(torch.pow(input=demean_target_y, exponent=2).sum(dim=1)))
