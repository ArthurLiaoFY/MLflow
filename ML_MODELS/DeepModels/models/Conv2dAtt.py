...
class Convolutional2DAttention(torch.nn.Module):
    def __init__(
        ...
        self.eqp_conv_block = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=self.in_channel_size, out_channels=self.in_channel_size, kernel_size=(num_of_eqp, 3), groups=self.in_channel_size, padding='same'),
            torch.nn.Conv2d(in_channels=self.in_channel_size, out_channels=self.out_channel_size, kernel_size=1, padding='same'),
        ...
        self.q_conv_layer = torch.nn.Sequential(
            torch.nn.Conv1d(in_channels=self.out_channel_size, out_channels=self.out_channel_size, kernel_size=3, padding='same'),
        ...
        self.linear = torch.nn.Linear(in_features=self.attention_embed_dim, out_features=self.cluster_num)

        self.final_layer = torch.nn.Sequential(
            torch.nn.Linear(in_features=self.additional_infos+self.out_channel_size * self.cluster_num, out_features=512),
    ...
    def forward(self, lagged_sn_count: torch.Tensor, lagged_infos: torch.Tensor, lagged_eqp_status_block: torch.Tensor):
        conv_result = self.eqp_conv_block(lagged_eqp_status_block)
        flatten_conv_result = torch.cat(
            tensors=[conv_result[:, :, idx, :] for idx in range(conv_result.size(2))], dim=-1
        )
        ...
        attn_output = torch.cat(tensors=(self.flatten(self.linear(attn_output+flatten_conv_result)), lagged_infos), dim=-1)
