...
class UnsupervisedDeepEmbeddingClustering(torch.nn.Module):
    ...
    def forward(self, encoded_tensor: torch.Tensor) -> torch.Tensor:
        sub_component = 1 + torch.pow(input=encoded_tensor - self.cluster_centers, exponent=2)/self.alpha
        t_distance = torch.pow(input=sub_component, exponent=-0.5*(self.alpha+1))/torch.pow(input=sub_component, exponent=-0.5*(self.alpha+1)).sum(dim=-1, keepdim=True)
        return t_distance
