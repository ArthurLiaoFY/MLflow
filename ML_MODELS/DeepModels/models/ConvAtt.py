...
class ConvolutionalAttention(torch.nn.Module):
    def __init__(
        ...
        self.eqp_conv_block = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=in_channel_size, out_channels=out_channel_size, kernel_size=3, padding='same'),
        ...
        self.q_conv_layer = torch.nn.Sequential(
            torch.nn.Conv1d(in_channels=num_of_eqp, out_channels=num_of_eqp, kernel_size=3, padding='same'),
        ...
        self.multihead_attention_block = torch.nn.MultiheadAttention(
            embed_dim=out_channel_size * num_of_status, num_heads=out_channel_size, batch_first=True, device=get_device()
        )
        self.final_layer = torch.nn.Sequential(
            torch.nn.Linear(in_features=3 + num_of_eqp * num_of_status * out_channel_size, out_features=256),
    ...
    def forward(self, lagged_infos: torch.Tensor, eqp_status_block: torch.Tensor):
        ...
        flatten_conv_result = torch.cat(
            tensors=[conv_result[:, idx, :, :] for idx in range(conv_result.size(1))], dim=-1
        )
        q, k, v = (
        ...
        attn_output = torch.cat(tensors=(self.flatten(attn_output+flatten_conv_result), lagged_infos), dim=-1)
