...
class Seq_ANOVA:
    ...
    def updatemodel(self, InfoName, X):
        ...
        if type(X) is dict:
            ...
            self.__Formula[InfoName] = self.__Formula[self.__PreviousInfoName] + ''.join(
                [' + ' + key + '_' + values for key, values in zip([Key] * len(Values), Values)])
        else:
            print("Type of X must be dict")
            return None
        updatedModel = LinearModel(self.__Formula[InfoName], DataFrame=self.__DataFrame)
        self.__DegOfFree[InfoName] = int(len(updatedModel.BetaHat) - sum(self.__DegOfFree.values()) - 1)
        self.__RSSDegOfFree[InfoName] = int(self.__RSSDegOfFree[self.__PreviousInfoName] - self.__DegOfFree[InfoName])
        self.__FittedValues[InfoName] = updatedModel.FittedValues
        self.__SumOfSquare[InfoName] = float(
            sum((updatedModel.FittedValues - self.__FittedValues[self.__PreviousInfoName]) ** 2))
        self.__RegSumOfSquare[InfoName] = float(sum(updatedModel.Residuals ** 2))
        self.FValue[InfoName] = float(
            (self.__RegSumOfSquare[self.__PreviousInfoName] - self.__RegSumOfSquare[InfoName]) / self.__DegOfFree[
                InfoName])
    ...
    def finish(self):
        ...
        for InfoName in self.FValue.keys():
            if InfoName != 'Intercept':
                ...
                self.PValue[InfoName] = 1 - stats.f.cdf(self.FValue[InfoName], self.__DegOfFree[InfoName], SecDegOfFree)
        ...
        self.AnovaTable = pd.concat([
            ...
            pd.DataFrame.from_dict(self.__SignificantStar, orient='index')], axis=1)
        self.AnovaTable.columns = ['Reg Df', 'Reg Sum of Sq', 'Df', 'Sum of Sq', 'F', 'Pr(>F)', ' ']
    ...
    def _get_ANOVA_summary_table(self):
        ...
        print('Response : {}, Number of Data : {}'.format(self.__Y, self.DataFrame.shape[0]), end=', ')
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(np.array([seed.choice(['AKCLA100', 'AKCLA200', 'AKCLA300', 'AKCLA400'], 2000),
                                ...
                                seed.choice(['Chamber_1', 'Chamber_2', 'Chamber_3', 'Chamber_4'], 2000)]).T,
                      columns=pd.MultiIndex.from_product([
                          ['GL_CLA', 'GL_SPT', 'GL_IEX', 'GL_WMA', 'GL_STO'],
                          ['EQP', 'CHAMBER']], names=['OP_ID', 'OP_Levels']))
    ...
    df.loc[(df[('GL_CLA',)] == ('AKCLA100', 'Chamber_2')).sum(axis=1) == 2, ('YLD', 'TOS1')] += 0.2

    ## Fit ANOVA ##
    anova = Seq_ANOVA(Y={'YLD': 'TOS1'}, DataFrame=df)
    anova.updatemodel('GL_CLA info', {'GL_CLA': {'EQP': None,
                                                 'CHAMBER': None}})
    anova.updatemodel('GL_SPT info', {'GL_SPT': {'EQP': None,
                                                 'CHAMBER': None}})
    anova.updatemodel('GL_IEX info', {'GL_IEX': {'EQP': None,
                                                 'CHAMBER': None}})
    anova.updatemodel('GL_WMA info', {'GL_WMA': {'EQP': None,
                                                 'CHAMBER': None}})
    anova.updatemodel('GL_STO info', {'GL_STO': {'EQP': None,
                                                 'CHAMBER': None}})
    anova.finish()
