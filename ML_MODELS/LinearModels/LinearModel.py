...
class LinearModel:
    ...
    def fit(self, x: np.ndarray | pd.DataFrame, y: np.ndarray | pd.DataFrame):
        ...
        self.x, self.y = self._add_intercept(
            self._prepare_input(x)
        ), self._prepare_input(y)
        ...
        self.cov_mat = np.sqrt(
            ...
            * (self.residuals.T @ self.residuals / (self.x.shape[0] - self.x.shape[1]))
        )
    ...
    def plot_residual(self):
        ...
        try:
            fig = go.Figure(
                go.Scatter(
                    x=list(range(len(self.residuals))), y=self.residuals, mode="markers"
                )
            )
...
def sum_coding_matrix(
    ...
    sum_coding_df_columns = [
        prefix + prefix_sep + unique_cat
        for unique_cat in unique_cats
        if unique_cat != baseline
    ]
    sum_coding = pd.DataFrame(
        columns=sum_coding_df_columns, data=np.zeros((len(data), len(unique_cats) - 1))
    )
