...
def to_tensor(x: np.ndarray | pd.DataFrame | torch.Tensor, device: str | None = None) -> torch.Tensor | None:
    ...
    match type(x):
        ...
        case _:
            try:
                ...
                print("X variable must be of one of the types: np.ndarray, pd.DataFrame, torch.Tensor.")
                tensor = None
...
def to_dataloader(
    ...
    return DataLoader(to_dataset(*data), batch_size=batch_size, shuffle=shuffle if sampler is None else False, drop_last=True)
