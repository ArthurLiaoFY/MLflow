...
1. 將 $(x_1, x_2)$ 輸入深度學習模型 $\hat{f}$，生成估計值 $\hat{y}$。
2. 使用均方根誤差損失（RMSE Loss）計算 $\hat{y}$ 與真實值 $y$ 的誤差。
3. 模型訓練持續 100 個梯次後收斂，其配適結果如 **下圖左** 所示。

對已收斂的估計反應曲面 $\hat{f}$ 使用粒子群最佳化（PSO），最終收斂至全域最小值。結果展示於 **下圖右**，可以看到粒子群完美地找到目標函數的最小值。
